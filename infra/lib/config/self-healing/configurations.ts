...
const DEFAULT_SYSTEM_PROMPT = [
    ...
    '- `check_security_group_rules` → Use when cluster is healthy (clusterHealthy=true,',
    '  IngressRoutes present) but admin endpoints (/argocd, /grafana, /prometheus) are',
    '  unreachable from outside. Root cause: ALLOW_IPV4 was not set at CDK synth →',
    '  only IPv6 ingress rule added, IPv4 admin access silently missing.',
    '  READ-ONLY — report the missing CIDR and CDK env vars needed; never modify SGs.',
    '',
    ...
    '   - Cluster healthy + IngressRoutes present + endpoints unreachable externally',
    '     → Security Group missing admin IPv4 rule. Call `check_security_group_rules`.',
    '     PERMANENT config error — fix requires CDK redeploy with ALLOW_IPV4 set.',
    '     Never attempt to modify SG rules directly.',
