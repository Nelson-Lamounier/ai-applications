...
// The user message is assembled section-by-section in `strategist-message.ts`.
// `STRATEGIST_MESSAGE_SECTIONS` there is the ordered toggle surface: tune,
// reorder, or disable ONE prompt section without touching the others. These
// re-exports keep every existing importer (callers + tests) unchanged.
export {
    buildStrategistMessage,
    CANDIDATE_CONTACT_HEADER,
    PROFILE_INTELLIGENCE_HEADER,
