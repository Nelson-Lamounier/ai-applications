...
describe('job-strategist run-pipeline.js — integration', () => {
    ...
    it('pipeline_runs metadata analysis has a cover letter', async () => {
        ...
        const coverLetter = analysis['coverLetter'] as Record<string, unknown> | null;
        ...
        expect(Array.isArray(coverLetter?.['paragraphs'])).toBe(true);
        expect((coverLetter?.['paragraphs'] as string[]).length).toBeGreaterThan(0);
    });
