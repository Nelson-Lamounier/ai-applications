 ...
 *  function stays usable in tests without a DB. */
...
export async function invokeClaude(
    ...
    if (costCtx) {
        recordBedrockCost(costCtx.pool, {
            ...
            modelId,
            pipeline:     'chatbot-authenticated',
            inputTokens:  response.usage?.inputTokens  ?? 0,
            outputTokens: response.usage?.outputTokens ?? 0,
        }).catch((err) => console.warn('[chatbot-authenticated] cost record failed (non-fatal)', err));
    }
