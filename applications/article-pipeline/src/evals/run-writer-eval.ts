/**
 * @format
 * Article-pipeline WRITER-phase eval.
 *
 ...
 * English, honest inline references, plausible reading time, valid slug).
 *
 * The scoring core is exported as {@link runWriterEval} for the in-cluster
 * orchestrator (run-evals.ts). The gated `main()` is the LOCAL ad-hoc path.
 *
 * Local run:
 *   RUN_ARTICLE_WRITER_EVAL=1 FOUNDATION_MODEL=eu.anthropic.claude-sonnet-4-6 \
...
export const WRITER_MIN_PASS = Number.parseFloat(process.env['WRITER_EVAL_MIN_PASS'] ?? '1.0');
...
/** Generate + grade every golden brief. Pure of process exit / gating. */
export async function runWriterEval(): Promise<WriterCheckReport[]> {
...
export function writerEvalPassRate(reports: ReadonlyArray<WriterCheckReport>): number {
    return reports.length === 0 ? 0 : reports.filter((r) => r.ok).length / reports.length;
}

async function main(): Promise<void> {
    if (process.env['RUN_ARTICLE_WRITER_EVAL'] !== '1') {
        console.log('Writer eval is gated. Set RUN_ARTICLE_WRITER_EVAL=1 (+ FOUNDATION_MODEL, AWS_REGION) to run, or dispatch the in-cluster eval Job.');
        return;
    }
    const reports = await runWriterEval();
    console.log('\n' + formatWriterReports(reports));
    const passRate = writerEvalPassRate(reports);
    const pass = passRate >= WRITER_MIN_PASS;
    console.log(`\n==> gate: clean-brief rate ≥ ${(WRITER_MIN_PASS * 100).toFixed(0)}% (got ${(passRate * 100).toFixed(0)}%) — ${pass ? 'PASS' : 'FAIL'}`);
