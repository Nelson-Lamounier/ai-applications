...
function getDefaultTools(): AgentTool[] {
    return [
        ...
        },
        {
            ...
            description: 'Verify Traefik IngressRoute and Middleware resources on the cluster. Use after a bootstrap alarm where step 7 (applyIngress) failed, or when applications return 404. Also checks for empty IPAllowList sourceRanges — note that empty sourceRanges immediately after bootstrap is expected (PostSync patcher fills them in after ArgoCD sync).',
            inputSchema: {
                type: 'object',
                properties: {
                    namespace: { type: 'string', description: 'Optional: scope to a single namespace (e.g. "argocd", "monitoring"). Omit for cluster-wide.' },
                },
            },
        },
        {
            name: 'check_cert_manager',
            description: 'Verify cert-manager ClusterIssuer and Certificate resources. Use when a bootstrap alarm fires for step 5d (applyCertManagerIssuer), when TLS certificates are missing, or when services return certificate errors. If no ClusterIssuers are found, check SSM paths — CDK stores them at edge/hosted-zone-id and edge/cross-account-role-arn.',
            inputSchema: {
                type: 'object',
                properties: {},
            },
        },
        {
            ...
            inputSchema: {
                type: 'object',
                properties: {
                    appName: { type: 'string', description: 'Optional: filter to a specific ArgoCD app name (e.g. "argocd-ingress", "monitoring")' },
                },
            },
        },
        {
...
function buildBootstrapDiagnosticGuidance(): string {
    return [
        ...
        '   - Step 5d (applyCertManagerIssuer) failed:',
        '     → Call `check_cert_manager` to verify ClusterIssuer presence and status.',
        '     → If no ClusterIssuers found: SSM paths may be wrong. CDK stores them at',
        '       edge/hosted-zone-id and edge/cross-account-role-arn (NOT public-hosted-zone-id).',
        '     → This is a PERMANENT config error — do NOT retry bootstrap; report to operator.',
        '   - Step 7 (applyIngress) failed:',
        '     → Call `check_argocd_sync` to verify argocd-ingress app sync status.',
        '     → Call `check_ingress_routes` to verify IngressRoutes were applied.',
        '     → If IngressRoutes are missing and ArgoCD app is OutOfSync, the Helm chart may',
        '       not have been applied yet — wait for ArgoCD sync or report to operator.',
        '     → Race condition (redundant ArgoCD pods guard) was fixed 2026-05-02.',
        '   - CALICO_TIMEOUT or CALICO_INIT:',
        '     → Call `inspect_workloads` with namespaces=["kube-system"] to check calico-node',
        '       DaemonSet pod status. A stuck Init:0/1 pod indicates BGP peer issue.',
        '     → TRANSIENT: safe to remediate_node_bootstrap.',
        '',
        '3. CLASSIFY the failure:',
        '   - TRANSIENT: Network timeouts, S3 eventual consistency, NLB propagation, Calico init',
        '     → Safe to retry by triggering `remediate_node_bootstrap`.',
        '   - PERMANENT: AMI mismatch, IAM permission denied, SSM path misconfiguration, bad config',
        ...
        '',
        '6. POST-BOOTSTRAP APPLICATION HEALTH (run after clusterHealthy = true):',
        '   - `check_ingress_routes` → confirm IngressRoutes for argocd, monitoring, traefik.',
        '     NOTE: empty IPAllowList sourceRanges immediately after bootstrap is EXPECTED',
        '     behaviour — the PostSync patcher Job fills them in after ArgoCD sync completes.',
        '     Do not flag this as an error unless it persists more than 5 minutes post-sync.',
        '   - `check_cert_manager` → confirm ClusterIssuer letsencrypt is Ready.',
        '   - `check_argocd_sync` → confirm all apps are Synced + Healthy.',
        '───────────────────────────────────────',
        '',
    ].join('\n');
}
