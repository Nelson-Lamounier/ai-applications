...
**Pure (unit-tested):**
- `buildJsonlRecords(items: { entry: RawImportEntry; ecosystem: string }[]): { records: BatchRecord[]; recordMap: Record<string, { ecosystem: string; identifier: string }> }` — pools entries across all sources, assigns each a **purely alphanumeric** `recordId` = `r` + zero-padded index (Bedrock requires `recordId` to match `^[a-zA-Z0-9]{1,64}$`, so the old `ecosystem:identifier` colon-encoding is illegal). Each record is `{ recordId, modelInput }`; `modelInput` = the Anthropic Messages body on Bedrock: `{ anthropic_version: "bedrock-2023-05-31", max_tokens: 256, system: <classifier system text>, tools: [classify_package], tool_choice: {type:"tool", name:"classify_package"}, messages: [{role:"user", content:<package summary>}] }`. (Carries over the existing system prompt, `classify_package` tool schema with the 30-category enum + null, and the package-summary message from `LlmBatchClassifier`. **No `cache_control`** — prompt caching is a runtime feature, unavailable in batch.) The `recordMap` (recordId → ecosystem+identifier) is persisted on the run so the follow-up can resolve outputs back to entries.
...
`findPendingBatches()` (now also returns each run's `notes.recordMap`) → for each, `retrieve(jobArn)`; `Completed` → `readResults` → `parseModelOutput` → resolve `recordId` via the run's `recordMap` to `{ecosystem, identifier}` → route → `finish('success')`; `Failed`/`Stopped` → `finish('failed')` (unresolved carry to next run); otherwise leave `partial`.
